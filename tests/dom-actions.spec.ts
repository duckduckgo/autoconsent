...
async function getButtonClickCount(page, id) {
  return await page.evaluate(
    (id) =>
      parseInt(document.querySelector(`#${id} > button`)?.innerHTML || "0", 10),
    id,
  );
}
...
test.beforeEach(async ({ page }) => {
  ...
  await page.evaluate(() => {
    document
      ...
      .forEach((el) =>
        el.addEventListener(
          "click",
          () => (el.innerText = JSON.stringify(parseInt(el.innerText, 10) + 1)),
        ),
      );
  });
  ...
  await page.evaluate(
    "window.domActions = new window.DomActions({ config: { logs: { lifecycle: false, rulesteps: false, evals: false, errors: false, messages: false } } })",
  );
});

test.describe("click", () => {
  test("click on button", async ({ page }) => {
    expect(await page.evaluate(() => window.domActions.click("#test"))).toBe(
      true,
    );
  ...
  test("click all", async ({ page }) => {
    expect(
      await page.evaluate(() => window.domActions.click("button", true)),
    ).toBe(true);
    expect(await getButtonClickCount(page, "first")).toEqual(1);
  ...
  test("click single with multiple matches", async ({ page }) => {
    expect(await page.evaluate(() => window.domActions.click("button"))).toBe(
      true,
    );
    expect(await getButtonClickCount(page, "first")).toEqual(1);
  ...
  test("chained selector", async ({ page }) => {
    expect(
      await page.evaluate(() => window.domActions.click(["#second", "button"])),
    ).toBe(true);
  ...
  test("xpath selector", async ({ page }) => {
    expect(
      await page.evaluate(() =>
        window.domActions.click(['xpath///*[@id="second"]/button']),
      ),
    ).toBe(true);
    expect(
      await page.evaluate(
  ...
  test("click open shadow dom element", async ({ page }) => {
    const shadowRoot = await page.evaluateHandle(() => {
      ...
      shadowButton.addEventListener(
        "click",
        () =>
          (shadowButton.innerText = JSON.stringify(
            parseInt(shadowButton.innerText, 10) + 1,
          )),
      );
      return shadow;
    });
    expect(
      await page.evaluate(() => window.domActions.click(["#shadow", "button"])),
    ).toBe(true);
    expect(
      await page.evaluate(
        () => document.querySelector("#first > button")?.innerHTML,
    ...
    expect(
      await page.evaluate(
        (shadow) => shadow.querySelector("button")?.innerHTML || "0",
        shadowRoot,
      ),
    ).toEqual("1");
  });
