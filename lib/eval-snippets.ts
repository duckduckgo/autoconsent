...
export const snippets = {
    ...
    /** True when the Ketch banner host exists and shows interactive buttons (works while host is pre-hidden with opacity 0). */
    EVAL_KETCH_POPUP_OPEN: () => {
        const host = document.getElementById('ketch-consent-banner');
    ...
    /** Ketch CMP (e.g. time.com): consent UI lives inside #ketch-consent-banner shadow root */
    EVAL_KETCH_OPT_OUT: () => {
        const host = document.getElementById('ketch-consent-banner');
        const root = host?.shadowRoot;
        if (!root) {
        ...
        for (const btn of buttons) {
            ...
            if (rejectPatterns.some((re) => re.test(text))) {
                btn.click();
                return true;
            }
        }
    ...
    EVAL_KETCH_TEST_DISMISSED: () => {
        const host = document.getElementById('ketch-consent-banner');
        if (!host) {
            return true;
        }
        const hostStyle = getComputedStyle(host);
        if (hostStyle.display === 'none' || hostStyle.visibility === 'hidden' || Number(hostStyle.opacity) === 0) {
            return true;
        }
        const root = host.shadowRoot;
        if (!root) {
