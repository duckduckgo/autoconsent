...
export const DETECT_PATTERNS = [
    ...
    // notices that describe cookies without naming them, e.g. "данные с использованием файлов, сохраняемых в браузере пользователя"
    /(?:файл[а-яё]*|данн[а-яё]*).{0,60}(?:сохраня[а-яё]*|сохранени[а-яё]*|хран[а-яё]*|размеща[а-яё]*).{0,30}(?:в\s+браузере|в\s+настройках\s+браузера|на\s+(?:вашем\s+)?устройстве)/gi,
    /(?:сохран[а-яё]*|хран[а-яё]*|размеща[а-яё]*).{0,30}файл[а-яё]*.{0,30}(?:в\s+браузере|в\s+настройках\s+браузера|на\s+(?:вашем\s+)?устройстве)/gi,
