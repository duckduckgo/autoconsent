...
export default class AutoConsentCMPBase implements AutoCMP, DomActionsProvider {
  ...
  querySingleReplySelector(selector: string, parent?: ParentNode) {
    return this.autoconsent.domActions.querySingleReplySelector(
      selector,
      parent,
    );
  }
...
export class AutoConsentCMP extends AutoConsentCMPBase {
  ...
  async evaluateRuleStep(rule: AutoConsentRuleStep) {
    ...
    if (rule.waitForVisible) {
      results.push(
        this.waitForVisible(rule.waitForVisible, rule.timeout, rule.check),
      );
    }
    if (rule.click) {
    ...
    if (rule.waitForThenClick) {
      results.push(
        this.waitForThenClick(rule.waitForThenClick, rule.timeout, rule.all),
      );
    }
    if (rule.wait) {
