...
export default class SourcePoint extends AutoConsentCMPBase {
  ...
  async detectCmp() {
    ...
    return (
      ...
      (url.searchParams.has("message_id") ||
        ...
        url.searchParams.has("consentUUID"))
    );
  }
  ...
  isManagerOpen() {
    return (
      ...
      location.pathname === "/ccpa_pm/index.html"
    );
  }
  ...
  async optOut() {
    ...
    if (!this.isManagerOpen()) {
      const actionable = await this.waitForElement(
        ".sp_choice_type_12,.sp_choice_type_13",
      );
      if (!actionable) {
        return false;
      }
    ...
    this.waitForThenClick(
      ".ccpa-stack .pm-switch[aria-checked=true] .slider",
      500,
      true,
    ); // the UI is reversed: "unchecked" switch displays as an enabled toggle
    ...
    try {
      ...
      const path = await Promise.race([
        this.waitForElement(rejectSelector1, 2000).then((success) =>
          success ? 0 : -1,
        ),
        this.waitForElement(rejectSelector2, 2000).then((success) =>
          success ? 1 : -1,
        ),
        this.waitForElement(".pm-features", 2000).then((success) =>
          success ? 2 : -1,
        ),
      ]);
      if (path === 0) {
