...
export default class TrustArc extends AutoConsentBase {
  ...
  async optOut(tab) {
    ...
    const success = await this.navigateToSettings(tab, frameId);
    if (success) {
      // select and submit
      await tab.clickElements('.switch span:first-child', frameId);
    } else {
      // slider, we have to send an arrow up event to move it to the top
      await tab.sendKeyEvent('.ui-slider-handle', 'keydown', 38, 0, frameId);
      await tab.sendKeyEvent('.ui-slider-handle', 'keydown', 38, 0, frameId);
    }
