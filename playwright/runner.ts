...
export function generateTest(
  ...
  function genTest(autoAction: AutoAction) {
    test(`${url.split("://")[1]} .${testRegion} ${autoAction} ${options.mobile ? "mobile" : ""}`, async ({
      page,
    }, { project }) => {
      if (
        options.onlyRegions &&
        options.onlyRegions.length > 0 &&
        !options.onlyRegions.includes(testRegion)
      ) {
        test.skip();
      }
      ...
      if (
        (options.mobile && !project.use.isMobile) ||
        (!options.mobile && project.use.isMobile)
      ) {
        test.skip();
      }
      ...
      function isMessageReceived(
        ...
        return received.some((m) => {
          const keysMatch =
            partial || Object.keys(m).length === Object.keys(msg).length;
          return (
            keysMatch &&
            (Object.keys(msg) as Array<keyof ContentScriptMessage>).every(
              (k) => m[k] === msg[k],
            )
          );
        });
      ...
      await waitFor(
        () => isMessageReceived({ type: "cmpDetected", cmp: expectedCmp }),
        50,
        500,
      );
      expect(isMessageReceived({ type: "cmpDetected", cmp: expectedCmp })).toBe(
        true,
      );

      await waitFor(
        () => isMessageReceived({ type: "popupFound", cmp: expectedCmp }),
        options.expectPopupOpen ? 50 : 5,
        500,
      );
      expect(isMessageReceived({ type: "popupFound", cmp: expectedCmp })).toBe(
        options.expectPopupOpen,
      );
      ...
      if (options.expectPopupOpen) {
        ...
        if (autoAction === "optIn") {
          await waitFor(
            () => isMessageReceived({ type: "optInResult", result: true }),
            50,
            300,
          );
          expect(isMessageReceived({ type: "optInResult", result: true })).toBe(
            true,
          );
        }
        if (options.testSelfTest && selfTestFrame) {
          await waitFor(
            () => isMessageReceived({ type: "selfTestResult", result: true }),
            50,
            300,
          );
          expect(
        ...
        await waitFor(
          () => isMessageReceived({ type: "autoconsentDone" }),
          10,
          500,
        );
