...
Search website source code via the [PublicWWW](https://publicwww.com) API. Requires `PUBLICWWW_APIKEY` env var.

## When to Use

- **Before creating a site-specific rule:** check if the popup's identifiers appear on other sites — if so, prefer a generic rule.
- **Finding test URLs:** discover more sites using a CMP, optionally filtered by TLD for region-specific URLs.
- **Investigating unknown popups:** search for selectors/script paths to identify the CMP provider.
...
```
https://publicwww.com/websites/SEARCH_QUERY/?export=csvsnippetsu&key=$PUBLICWWW_APIKEY
```
...
```bash
curl -s "https://publicwww.com/websites/%22sd-cmp%22/?export=csv&key=$PUBLICWWW_APIKEY" | head -20
```

Find German sites using OneTrust:

```bash
curl -s "https://publicwww.com/websites/%22onetrust-banner-sdk%22+site%3Ade/?export=csv&key=$PUBLICWWW_APIKEY" | head -10
```

Search JS files for a CMP API:

```bash
curl -s "https://publicwww.com/js/%22__cmp%22+filetype%3Ajs/?export=csv&key=$PUBLICWWW_APIKEY" | head -10
```
